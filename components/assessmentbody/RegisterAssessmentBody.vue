         ...
         <b-link @click="$emit('changeComponent',{component: 'AssessmentBodyView', id: null})"  href="#" class="card-link text-white"><mdb-icon icon="arrow-left" size="lg" class="text-white" />  View All Assessment Bodies</b-link>
            ...
            <mdb-card class="weather-card">
              ...
              <b-alert v-if="errorState" show variant="danger">Training Provider  Failed to Create. Try again</b-alert>         
                <h2 class="h2-responsive"><strong>New Assessment Body Form</strong></h2>
                <p class="pb-4">Create Assessment Body Profile</p>
                <!--Body-->
                <form>
                  <mdb-input label="Full Name"  v-model="assessmentProfileForm.fullname" />
                  <mdb-input label="Accronym"   v-model="assessmentProfileForm.accronym"/>
                  <mdb-input label="Telephone Number"  v-model="assessmentProfileForm.telephoneNumber"/>
                  <mdb-input label="Email Address"  v-model="assessmentProfileForm.emailAddress"/>
                  <mdb-input label="Website"  v-model="assessmentProfileForm.websiteAddress"/>
                  <mdb-input label="Contact Address"  v-model="assessmentProfileForm.contactAddress"/>
                  <mdb-input label="City"  v-model="assessmentProfileForm.city"/>
                  <mdb-input label="Postal Code"  v-model="assessmentProfileForm.postalCode"/>
                  <mdb-input label="Mission Statement"  v-model="assessmentProfileForm.missionStatment"/>
                  ...
                  <div class="text-xs-left">
                    <mdb-btn color="primary" @click.native.prevent="create()" :disabled='assessmentProfileFormReset'>Submit
                      <b-spinner small v-if="assessmentProfileFormReset === true"></b-spinner>
                      <span class="sr-only" v-if="assessmentProfileFormReset === true">Wait...</span>
                    </mdb-btn>
                  </div>
                </form>
              </mdb-card-body>
   ...
   components: {
    ...
    data() {
      return {
        assessmentProfileForm: {
            ...
            image: null,
            resultURL: '',
            assessmentProfileFormReset: false,
            spinner: '0'        
          }
    },
    computed: {
    ...
    methods: {
      ...
      if(!this.licensingProfileForm.fullname) {
        ...
        this.assessmentProfileFormReset = !this.assessmentProfileFormReset
          let uuid = uuidv4();
          let logoURL = ''
