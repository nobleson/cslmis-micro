...
<div>
    <b-row>
            <b-col align-self="end"  class="pb-3 pl-auto" cols="12">
                <mdb-btn size="lg" color="primary"  @click="$emit('changeComponent',{component: 'NewReport', id: null})" class="text-white primary-color-dark">Create New</mdb-btn>
             <hr>
            </b-col>
            <hr>
      </b-row>
  ...
  <mdb-col class="m-5" cols="12">
    ...
    <mdb-tab-content>
      <mdb-tab-pane class="fade" key="show1" v-if="pillsActive==0">
        <mdb-row>
              <b-col v-for="body in processReport" :key="body.id" cols="3">
                <div>
                  <mdb-card wide>
                    <mdb-card-up style="background-color: #2BBBAD">
                    </mdb-card-up>
                    <mdb-card-body class="text-center pb-0" cascade>
                      <mdb-card-title><strong>{{body.reportTitle}} </strong></mdb-card-title>
...
export default {
   ...
   data() {
    ...
    computed: {
          ...mapGetters({reports: 'reports/getReports',isContentLoading:'reports/getLoaderStatus'}),
          processReports: function(){
            if(typeof this.reports[0] != 'undefined' || this.reports[0] != null){
               this.dataObject = this.reports[0]
               this.reportList = this.dataObject.reportList
               return this.reportList = this.dataObject.reportList
            }
          ...
          processTableData: function(){
            if(Array.isArray(this.reportList)){
              this.dataObject = this.reports[0]
              this.dataSet.rows = this.dataObject.reportList
              return  this.dataSet;
            }else{
              this.dataSet.rows = this.reports.reportList
              return  this.dataSet;
            }
    ...
    methods: {
      ...mapActions({loadReports: 'reports/loadReports'}),
        create() {
          this.loadReports().then(function (e){console.log("successfully load")}).catch(function(error){console.log(error.message)});
        },
      linkClass(idx) {
