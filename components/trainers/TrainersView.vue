...
<div>
    <b-row>
            <b-col align-self="end"  class="pb-3 pl-auto" cols="12">
                <mdb-btn size="lg" color="primary"  @click="$emit('changeComponent',{component: 'NewTrainers', id: null})" class="text-white primary-color-dark">Create New</mdb-btn>
             <hr>
            </b-col>
            <hr>
      </b-row>
  ...
  <mdb-col class="m-5" cols="12">
    ...
    <mdb-tab-content>
      <mdb-tab-pane class="fade" key="show1" v-if="pillsActive==0">
        <mdb-row>
              <b-col v-for="body in processTrainers" :key="body.id" cols="3">
                <div>
                  <mdb-card wide>
...
export default {
   ...
   data() {
    ...
    computed: {
          ...mapGetters({trainers: 'trainer/getTrainers',isContentLoading:'trainer/getLoaderStatus'}),
          processTrainers: function(){
            if(typeof this.trainers[0] != 'undefined' || this.trainers[0] != null){
               this.dataObject = this.trainers[0]
               this.trainerList = this.dataObject.trainerList
               return this.trainerList = this.dataObject.trainerList
            }
          ...
          processTableData: function(){
            if(Array.isArray(this.trainers)){
              this.dataObject = this.trainers[0]
              this.dataSet.rows = this.dataObject.trainerList
              return  this.dataSet;
            }else{
              this.dataSet.rows = this.trainers.trainerList
              return  this.dataSet;
            }
    ...
    methods: {
      ...mapActions({loadTrainers: 'trainer/loadTrainers'}),
        create() {
          this.loadTrainers().then(function (e){console.log("successfully load")}).catch(function(error){console.log(error.message)});
        },
      linkClass(idx) {
