                ...
                <div>
                  <mdb-card wide>
                    ...
                    <mdb-card-body class="text-center pb-0" cascade>
                      <mdb-card-title><strong>{{body.aprCenterName}} </strong></mdb-card-title>
                      <h5 class="blue-text"><strong>{{body.idNumber}}</strong></h5>
                      <mdb-card-text>{{body.contactAddress}}</mdb-card-text>
                    </mdb-card-body>
                  </mdb-card>
                  </div>
                </b-col>
...
export default {
   ...
   data() {
    ...
    computed: {
          ...mapGetters({facilities: 'aprCenters/getAprCenters',isContentLoading:'aprCenters/getLoaderStatus'}),
          processAprCenters: function(){
            if(typeof this.aprCenters[0] != 'undefined' || this.aprCenters[0] != null){
               this.dataObject = this.aprCenters[0]
               this.apprentishipCenterList = this.dataObject.apprentishipCenterList
               return this.apprentishipCenterList = this.dataObject.apprentishipCenterList
            }
            //if(Array.isArray(this.trainees)){}
          },
          processTableData: function(){
            if(Array.isArray(this.apprentishipCenterList)){
              this.dataObject = this.aprCenters[0]
              this.dataSet.rows = this.dataObject.apprentishipCenterList
              return  this.dataSet;
            }else{
              this.dataSet.rows = this.aprCenters.apprentishipCenterList
              return  this.dataSet;
            }
    ...
    methods: {
      ...mapActions({loadAprCenters: 'aprCenters/loadAprCenters'}),
        create() {
          this.loadAprCenters().then(function (e){console.log("successfully load")}).catch(function(error){console.log(error.message)});
        },
      linkClass(idx) {
