...
<div>
    <b-row>
            <b-col align-self="end"  class="pb-3 pl-auto" cols="12">
                <mdb-btn size="lg" color="primary"  @click="$emit('changeComponent',{component: 'NewTrainees', id: null})" class="text-white primary-color-dark">Create New</mdb-btn>
             <hr>
            </b-col>
            <hr>
      </b-row>
  ...
  <mdb-col class="m-5" cols="12">
    ...
    <mdb-tab-content>
      <mdb-tab-pane class="fade" key="show1" v-if="pillsActive==0">
        <mdb-row>
              <b-col v-for="body in processTrainees" :key="body.id" cols="3">
                <div>
                  <mdb-card wide>
...
export default {
   ...
   data() {
    ...
    computed: {
          ...mapGetters({trainees: 'trainee/getTrainees',isContentLoading:'trainee/getLoaderStatus'}),
          processTrainees: function(){
            if(typeof this.trainees[0] != 'undefined' || this.trainees[0] != null){
               this.dataObject = this.trainees[0]
               this.traineeList = this.dataObject.traineeList
               return this.traineeList = this.dataObject.traineeList
            }
          ...
          processTableData: function(){
            if(Array.isArray(this.trainees)){
              this.dataObject = this.trainees[0]
              this.dataSet.rows = this.dataObject.traineeList
              return  this.dataSet;
            }else{
              this.dataSet.rows = this.trainees.traineeList
              return  this.dataSet;
            }
    ...
    methods: {
      ...mapActions({loadTrainees: 'trainee/loadTrainees'}),
        create() {
          this.loadTrainees().then(function (e){console.log("successfully load")}).catch(function(error){console.log(error.message)});
        },
      linkClass(idx) {
