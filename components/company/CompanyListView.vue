...
<section>
    <b-row>
            <b-col align-self="end"  class="pb-3 pl-auto" cols="12">
                <mdb-btn size="lg" color="primary"  @click="$emit('changeComponent',{component: 'NewCompany', id: null})" class="text-white primary-color-dark">Create New</mdb-btn>
             <hr>
            </b-col>
            <hr>
      </b-row>
  ...
  <mdb-col class="m-5" cols="12">
    ...
    <mdb-tab-content>
      <mdb-tab-pane class="fade" key="show1" v-if="pillsActive==0">
        <mdb-row>
            <b-col v-for="body in processCompanies" :key="body.id" cols="4">
            <div>
              <mdb-card wide>
                <mdb-card-up style="background-color: #2BBBAD">
                  <img src="../../assets/images/company.png" width="100%" max-height="150px" />
                </mdb-card-up>
                ...
                <mdb-card-body class="text-center pb-0" cascade>
                  <mdb-card-title><strong>{{body.companyName}}</strong></mdb-card-title>
                  <h5 class="blue-text"><timeago :datetime="body.dateRegistered"></timeago></h5>
                  <mdb-card-text>{{body.companyAcronym}}</mdb-card-text>
                  <mdb-card-footer class="text-muted mt-4"><mdb-btn outline="default" darkWaves size="lg" @click="$emit('changeComponent',{component:'CompanyDetails',data: body})" >Manage</mdb-btn></mdb-card-footer>
                </mdb-card-body>
              </mdb-card>
              </div>
            </b-col>
...
export default {
   ...
   data() {
    ...
    computed: {
          ...mapGetters({companies: 'company/getCompanies', isContentLoading: 'company/getLoaderStatus'}),
          processCompanies: function(){
          let json = JSON.parse(JSON.stringify(this.companies));
         // console.log("Json:"+JSON.stringify(this.companies));
           return  json;
          },
          processTableData: function(){
          this.dataSet.rows = this.companies;
