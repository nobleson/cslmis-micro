...
<!--made sure every form is bound to its right form object using the index-->
    ...
    <label class="custom-control-label" for="defaultGroupExample2">Trade Learnt</label>
     ...
     <tab-content title="Artsan Education"
      ...
      <div>
    <b-form v-if="show">
   <b-button type="button" variant="success"  @click="addEducationItem">Add Education</b-button>
<!--added index to track every form with its index.-->
...
<!--made sure every form is bound to its right form object using the index-->
    <div class="flex-row">
     ...
     <tab-content title="Artisan Employment History"
       ...
       <div>
    <b-form  v-if="show">
      <label>Trade Practice</label>
     <b-form-select v-model="employmentForm.trade" :options="options" multiple :select-size="4"></b-form-select>

        <mdb-input
          id="yearsPractice"
          v-model="employmentForm.yearsPractice"
          required
        ...
        <mdb-input
          id="siteAddress"
          v-model="employmentForm.siteAddress"
          required
          placeholder="Current Project Site Address (if any)"
        ></mdb-input>
   

        <mdb-input
          id="currentEmploymentStatus"
          v-model="employmentForm.currentEmploymentStatus"
          required
          placeholder="Enter Current Employment Status"
        ></mdb-input>


   <b-button type="button" variant="success"  @click="addWorkItem">Add Work Expirience</b-button>
<!--added index to track every form with its index.-->
  <mdb-card class="card-body" style="margin-top: 1rem;" v-for="(item, index) in employmentForm.workExpirience" v-bind:key="item.id">
    <mdb-card-title>Work Expirience {{index + 1}}
      <button v-if="index != 0" @click="removeEducation(index)" type="button" class="close" aria-label="Close">
      <span aria-hidden="true">×</span>
    </button>
    </mdb-card-title>
<!--made sure every form is bound to its right form object using the index-->
    <div class="flex-row">
       <mdb-input label="Company Name" v-model="employmentForm.workExpirience[index].companyName" size="lg" required/>
      <mdb-input label="Date Started" v-model="employmentForm.workExpirience[index].dateStart" size="lg" required/>
      <mdb-input label="Date Ended" v-model="employmentForm.workExpirience[index].dateEnd" size="lg" required/>
      <mdb-input label="Job Role" v-model="employmentForm.workExpirience[index].jobRole" size="lg" required/>
      <mdb-input label="Supervisor Name" v-model="employmentForm.workExpirience[index].supervisorName" size="lg" required/>
      <mdb-input label="Supervisor Phone Number" v-model="employmentForm.workExpirience[index].supervisorPhoneNumber" size="lg" required/>
    </div>
  </mdb-card>
       
     <b-card class="mt-3" header="Form Data Result">
      <pre class="m-0">{{ employmentForm }}</pre>
    </b-card>
           

      <b-button type="button" variant="primary" @click="saveEmployment">Submit</b-button>
      <b-button type="reset" variant="danger">Reset</b-button>
    </b-form>
