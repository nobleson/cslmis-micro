...
<section>
    <b-row>
            <b-col align-self="end"  class="pb-3 pl-auto" cols="12">
                <mdb-btn size="lg" color="primary"  @click="$emit('changeComponent',{component: 'NewLicensingBody', id: null})" class="text-white primary-color-dark">Create New</mdb-btn>
             <hr>
            </b-col>
            <hr>
      </b-row>
  ...
  <mdb-col class="m-5" cols="12">
    ...
    <mdb-tab-content>
      <mdb-tab-pane class="fade" key="show1" v-if="pillsActive==0">
        <mdb-row>
                <b-col v-for="body in processLicensingBody" :key="body.id" cols="4">
                <div>
                  <mdb-card wide>
                    <mdb-card-up style="background-color: #2BBBAD">
                      <img src="../../assets/images/licbdy.png" width="100%" max-height="150px" />
                    </mdb-card-up>
...
export default {
   ...
   data() {
    ...
    computed: {
          ...mapGetters({licensingBodies: 'licensingbody/getLicensingBodies', isContentLoading: 'licensingbody/getLoaderStatus'}),
    ...
    methods: {
      ...mapActions({loadLicensingBodies: 'licensingbody/loadLicensingBodies'}),
        create() {
