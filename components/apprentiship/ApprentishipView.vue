...
<div>
    <b-row>
            <b-col align-self="end"  class="pb-3 pl-auto" cols="12">
                <mdb-btn size="lg" color="primary"  @click="$emit('changeComponent',{component: 'NewApprentiship', id: null})" class="text-white primary-color-dark">Create New</mdb-btn>
             <hr>
            </b-col>
            <hr>
      </b-row>
  ...
  <mdb-col class="m-5" cols="12">
    ...
    <mdb-tab-content>
      <mdb-tab-pane class="fade" key="show1" v-if="pillsActive==0">
        <mdb-row>
              <b-col v-for="body in processApprentice" :key="body.id" cols="3">
                <div>
                  <mdb-card wide>
...
export default {
   ...
   data() {
    ...
    computed: {
          ...mapGetters({apprentice: 'apprentice/getApprentice',isContentLoading:'apprentice/getLoaderStatus'}),
          processApprentice: function(){
            if(typeof this.apprentice[0] != 'undefined' || this.apprentice[0] != null){
               this.dataObject = this.apprentice[0]
               this.apprenticeList = this.dataObject.apprenticeList
               return this.apprenticeList = this.dataObject.apprenticeList
            }
            //if(Array.isArray(this.trainees)){}
          },
          processTableData: function(){
            if(Array.isArray(this.apprentice)){
              this.dataObject = this.apprentice[0]
              this.dataSet.rows = this.dataObject.apprenticeList
              return  this.dataSet;
            }else{
              this.dataSet.rows = this.apprentice.apprenticeList
              return  this.dataSet;
            }
    ...
    methods: {
      ...mapActions({loadApprentice: 'apprentice/loadApprentice'}),
        create() {
          this.loadApprentice().then(function (e){console.log("successfully load")}).catch(function(error){console.log(error.message)});
        },
      linkClass(idx) {
